...
function serializeLicense(row) {
  ...
  const licensePlan = normalizeLicensePlanForType(licenseType, row.license_plan);
  const cloudPlayerSlots = normalizeCloudPlayerSlots(licensePlan, row.cloud_player_slots);
  ...
  return {
    ...
    licensePlan,
    licensePlanLabel: getLicensePlanDefinition(licensePlan).label,
    cloudPlayerSlots,
    cloudEnabled: licensePlan === "pro" && cloudPlayerSlots > 0,
...
function createLicense(input = {}) {
  ...
  const licensePlan = normalizeLicensePlanForType(licenseType, input.licensePlan);
  const cloudPlayerSlots = normalizeCloudPlayerSlots(licensePlan, input.cloudPlayerSlots);
...
function updateLicense(licenseId, updates = {}) {
  ...
  const currentLicensePlan = normalizeLicensePlanForType(currentLicenseType, existing.license_plan);
  const nextLicenseType = updates.licenseType !== undefined
    ? normalizeLicenseType(updates.licenseType, currentLicenseType)
    : currentLicenseType;
  const nextLicensePlan = updates.licensePlan !== undefined
    ? normalizeLicensePlanForType(nextLicenseType, updates.licensePlan)
    : normalizeLicensePlanForType(nextLicenseType, currentLicensePlan);
  const cloudPlayerSlots = updates.cloudPlayerSlots !== undefined || nextLicensePlan !== currentLicensePlan
    ? normalizeCloudPlayerSlots(nextLicensePlan, updates.cloudPlayerSlots)
    : normalizeCloudPlayerSlots(nextLicensePlan, existing.cloud_player_slots);
...
function activateLicense(payload = {}) {
  ...
  return {
    ...
    licensePlan: normalizeLicensePlanForType(result.refreshedLicense.license_type, result.refreshedLicense.license_plan),
    cloudPlayerSlots: normalizeCloudPlayerSlots(
      normalizeLicensePlanForType(result.refreshedLicense.license_type, result.refreshedLicense.license_plan),
      result.refreshedLicense.cloud_player_slots
    )
  };
}
...
function assertPluginLicenseSession(payload = {}) {
  reconcileSessions();

  const licenseKey = normalizeText(payload.licenseKey).toUpperCase();
  const username = normalizeIdentifier(payload.username);
  const sessionToken = normalizeText(payload.sessionToken);

  if (!licenseKey) {
    throw new HttpError(400, "A license key is required.");
  }
  if (!username) {
    throw new HttpError(400, "A username is required.");
  }
  ...
  if (!license.customer_username || normalizeIdentifier(license.customer_username) !== username) {
    throw new HttpError(403, "The provided username does not match this license.");
  }
  if (!license.active) {
    throw new HttpError(403, "This license is disabled.");
  ...
  if (session.license_id !== license.id) {
    throw new HttpError(403, "Session does not belong to this license.");
  }

  return { license, session };
}
